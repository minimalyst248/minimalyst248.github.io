...
$(document).ready(() => {
  const links = [
    {
      name: 'facebook',
      link: 'https://www.facebook.com/alice.inwonderland.912',
    },
    {
      name: 'notion',
      link: 'https://gentle-antelope-142.notion.site/Wonder-Land-cec337cfc0af4c69a8f52cc9e52bed85',
    },
    {
      name: 'discord',
      link: '',
  ...
  app.titleChanger(['anh', 'yêu', 'em', 'nhiều', 'lắm', 'bé', 'con', 'ạ']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Em à', " <span style='font-size: 16px; color: #e6d7bd;'>Cát Tường</span> đó hả ?"], 25, () => {
    ...
    writeLine([`<span style='font-size: 30px; color: #e6d7bd;'>Cát Tường ngokngeck</span>`, `Anh có làm cái website này vui vui trong lúc em ngủ cho em`], 20, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>từ từ rùi anh sẽ cập nhật thêm em nhé ❤❤❤ </i>`], 30, 700, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
